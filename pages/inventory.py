...
class Inventory(UserControl):
    def __init__(self, page):
        ...
        self.inventory_name= TextField(label='Vendor Name')
        self.inventory_date= TextField(label=f'{date.today()}',read_only=True)
        self.inventory_vendor= TextField(label='Vendor Name')
        self.inventory_website= TextField(label='Vendor Website')
        self.inventory_submit= ElevatedButton(text='Add to Database', on_click=lambda _: self.add_inventory() )
    def add_inventory(self):
        self.crud=CRUD()
        self.crud.create(Inventory(inventory_name=self.inventory_name.value,
                                inventory_vendor=self.inventory_vendor.value,
                                inventory_website=self.inventory_website.value,
          ...
          return Column(
            controls=[
              ...
              Container(
                  ...
                  content=Column(controls=[self.inventory_name,self.inventory_website,self.inventory_date,self.inventory_vendor,self.inventory_submit])
              )
                  ]
          )
              )
